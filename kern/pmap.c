...
mem_init(void)
{
	...
	// Jacky 140103 test the limitation of boot_alloc()
//	kern_pgdir = (pde_t *) boot_alloc(PGSIZE * NPTENTRIES);
	...
	// Jacky kern_pgdir[PDX(KERNBASE - 12M) = PDX(0xefc00000) = 3bf] = PADDR(0Xf0119000) | 0x1 | 0x4 = 0x119005;
...
page_alloc(int alloc_flags)
{
	...
	if (page_free_list == NULL) {
		...
		page_free_list = NULL;
		cprintf("Warning: You get the last page of memory!\n");
	} else {
...
page_free(struct Page *pp)
{
	// Jacky 140103
	if (pp->pp_ref != 0) {
		cprintf("Some one still use this page, could not free, \
				ref = %d, addr = 0x%x",pp->pp_ref, page2kva(pp));
		return;
	}
	if (!page_free_list) {
		page_free_list = pp;
...
// 	cprintf("check_va2pa(kern_pgdir, PGSIZE) = 0x%x, page2pa(%x) = 0x%x!\n",check_va2pa(kern_pgdir, PGSIZE),pp2, page2pa(pp2));
	...
	//cprintf("%s,%d, ptep = 0x%x, kern_pgdir = 0x%x\n",__FILE__,__LINE__,ptep,kern_pgdir);
